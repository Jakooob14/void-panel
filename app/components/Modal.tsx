...
export function Modal({ children, isOpen = true, locked = false, onClose, title, primaryButton, exitButton }: ModalProps) {
  ...
  return createPortal(
    <motion.div className={'fixed z-[100] top-0 left-0 w-screen h-screen overflow-hidden'} initial={{ opacity: 0 }} animate={{ opacity: opacity }} transition={{ duration: 0.2 }}>
      <div className={'absolute w-full h-full bg-alt-gray-100 opacity-70'} onClick={(e) => !locked && handleClose(e)}></div>
      <div className={'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 min-w-[500px] min-h-[200px] bg-alt-gray-200 shadow-lg p-6 flex flex-col justify-between'}>
        <div>
