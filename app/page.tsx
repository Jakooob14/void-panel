            ...
            files.map((file: { fileName: string; fileId: string }, index) => (
              <li className={'w-[320px] h-[275px] bg-alt-gray-100 flex flex-col justify-between p-4 gap-4'} key={index}>
                ...
                <div className={'flex justify-end gap-2'}>
                  ...
                  <Link className={'text-white text-xl'} href={`/${file.fileId}`}>
                    <FaLink />
                  </Link>
...
export function DeleteButton({ children, id, onDelete, className }: DeleteProps) {
  ...
  return (
    <button
      className={className}
      onClick={() =>
        showModal(false, 'Opravdu si přejete smazat tento soubor?', 'Opravdu?', <AnchorButton onClick={handleDelete}>Vymáznout</AnchorButton>, <button className={'px-4 w-full h-full'}>Zrušit</button>)
      }
    >
      {children}
    </button>
